<template>
  <div>
            ...
            <el-col :key="index" :span="12">
              <el-form-item :label="labels[formitem.ColName]" :class="{'is-error':formitem.AnalyseResult==0}">
                <el-input v-model="form[formitem.ColName]" :rows="4" :type="(formitem.ColName=='CollegeFullPath'||formitem.ColName=='ConstructFullPath')?'textarea':'text'">
                  ...
                  <i v-if="formitem.ColName=='BedName'" slot="suffix" class="el-input__icon el-icon-edit" @click="addBed"></i>
                </el-input>
    ...
    <el-dialog title="选择入住人员" :visible.sync="userVisible" :modal="false" width="1200px" @before-close="userVisible=false">
    ...
    <el-dialog title="选择入住床位" :visible.sync="bedVisible" :modal="false" width="1200px" @before-close="bedVisible=false">
      <el-scrollbar style="height:500px;margin: -20px 0;">
        <SelectBed ref="selectBed" is-user="0" :append="false" :show-switch="false" @change="change"></SelectBed>
      </el-scrollbar>
      <div slot="footer" class="dialog-footer">
        <el-button @click="bedVisible = false">取 消</el-button>
        <el-button type="primary" @click="bedComfirm">确 定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
<script>
import {
  BatchSetRZTemporaryTable,
  GetRZAnalyseResultInfo
} from '@/api/LiveInfo'
  ...
  methods: {
    ...
    bedComfirm() {
      ...
      GetBeds({
        ...
        var bed = res.Data[0]
        this.form.BedId = bed.Id
        this.form.BedName = bed.BedName
        this.form.BedSex = bed.BedSex_CN
        this.form.LiveState = bed.LiveState
        this.form.ConstructFullPath = bed.ConstructFullPath
    ...
    userComfirm() {
      ...
      this.form.UserId = this.currentRow.Id
      this.form.UserName = this.currentRow.Name
      this.form.UserSex = this.currentRow.Sex_CN
      this.form.UserNumber = this.currentRow.Number

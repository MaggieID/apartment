...
<script>
import { ExcelConstructInfo, ExcelUserInfo } from '@/api/LiveInfo'
  ...
  methods: {
    ...
    ExcelUserInfo() {
      ...
      ExcelUserInfo({
        ...
        this.close()
        fileDownload(res, '入住模板-学院' + parseTime(new Date()) + '.xlsx')
      })
    },
    ExcelConstructInfo() {
      this.downloadLoading = true
      ...
      ExcelConstructInfo({
        ...
        this.downloadLoading = false
        this.close()
        fileDownload(res, '入住模板-楼栋' + parseTime(new Date()) + '.xlsx')
      })
    },
    getSex(val) {

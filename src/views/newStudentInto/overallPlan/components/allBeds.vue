  ...
  <div>
    <el-drawer :modal="false" size="800px" :visible.sync="value" :before-close="handleClose">
      <div v-if="distribution==2" slot="title">
        <span style="color: black;padding-right: 15px;">床位数</span>
      ...
      <div v-if="distribution==0" slot="title">
        <span style="color: black;">未分配床位信息</span>
      </div>
      <div v-if="distribution==1" slot="title">
        <span style="color: black;">添加入住床位</span>
      </div>
...
export default {
  components: { pagination, bedDetail, Treeselect },
  props: {
    value: Boolean,
    distribution: {
      type: Number,

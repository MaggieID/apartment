<template>
  <div>
    <el-dialog :visible.sync="value" :title="'学生'+addSign?'新增':'修改'" :before-close="cancel" :modal="false" width="1000px">
      <el-form ref="form" :model="form" :rules="formRule" label-position="top">
        <el-tabs v-model="activeName" type="card">
          <el-tab-pane label="基本信息" name="1">
            <div style="height:460px" class="modifyDialog">
              <el-scrollbar style="height:100%">
                <el-row :gutter="20">
                  ...
                  <el-col :span="8">
                    <el-form-item label="入学时间" prop="StartSchoolTime">
                      <el-date-picker v-model="form.StartSchoolTime" type="date" value-format="yyyy-MM-dd" placeholder="选择日期" />
                    </el-form-item>
                  </el-col>
                  <el-col :span="8">
                    <el-form-item label="毕业时间" prop="GraduateTime">
                      <el-date-picker v-model="form.GraduateTime" type="date" value-format="yyyy-MM-dd" placeholder="选择日期" />
                    </el-form-item>
                  </el-col>
                  <el-col :span="8">
                    <el-form-item label="学制" prop="StudyingYear">
                      <el-select v-model="form.StudyingYear" filterable clearable>
                        <el-option v-for="item in data.XZList" :key="item.Code" :label="item.CName" :value="item.Code"></el-option>
                      </el-select>
                    </el-form-item>
                  </el-col>
                  <el-col :span="8">
                  ...
                  <el-col :span="8">
                    <el-form-item label="学生状态" prop="StuSatus">
                      <treeselect v-model="form.StuSatus" clearable :options="data.XSDQZTList" :normalizer="normalizer" placeholder="请选择" />
                    </el-form-item>
                  </el-col>
                  <el-col :span="8">
                    <el-form-item label="学生类别" prop="StuCategory">
                      <treeselect v-model="form.StuCategory" clearable :options="data.XSLBList" :normalizer="normalizer" placeholder="请选择" />
                    </el-form-item>
                  </el-col>
                  <el-col :span="8">
          ...
          <el-tab-pane label="紧急联系人" name="2">
            <div style="height:460px">
              <el-scrollbar style="height:100%">
                <el-table :data="EcPs" border header-row-class-name="table" style="width: 100%;line-height: normal;">
                  ...
                  <el-table-column label="操作" align="center" width="85">
                    <template slot-scope="scope">
                      <el-button type="text" icon="el-icon-delete" title="删除" class="table-btn btn-del" @click="delfunc(scope.$index)" />
                    </template>
                  </el-table-column>
                </el-table>
                <div style=" width: 100%; text-align: center;  line-height: 50px; border: 1px solid #dfe6ec; border-top: 0;" @click="addfunc">
                  添加联系人
                </div>
              </el-scrollbar>
            </div>
...
export default {
  ...
  methods: {
    ...
    delfunc(index) {
      this.EcPs.splice(index, 1)
    },
    addfunc() {
      for (let i = 0; i < this.EcPs.length; i++) {
        if (this.EcPs[i].name == '') {
          this.$message({
            message: '请输入紧急联系人姓名',
            type: 'warning'
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          var form = {
            ...
            BusinessActivation: data.BusinessActivation,
            Filing: data.Filing,
            ...
            Race: data.Race,
            Marriage: data.Marriage == null ? '' : data.Marriage,
            ...
            EcPs: this.EcPs,
            StartSchoolTime: data.StartSchoolTime,
            StudyingYear: data.StudyingYear,
            StuCategory: data.StuCategory == null ? '' : data.StuCategory,
            StuSatus: data.StuSatus == null ? '' : data.StuSatus,

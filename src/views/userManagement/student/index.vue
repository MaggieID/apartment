<template>
  <div class="g-container">
    <el-scrollbar style="height:100%">
      <el-card shadow="never" class="m-search">
        ...
        <div style="text-align: right;">
          ...
          <DisplayFields ref="searchField" title="更多查询字段" left-icon="el-icon-s-operation" api-name="student_query" @GetUserDisplay="getCondition" />
      ...
      <el-card style="margin-top:15px" shadow="never">
        <el-form ref="form" :inline="true">
          ...
          <div class="mb15">
            ...
            <DisplayFields title="更多表格字段" api-name="student_display" show-sort left-icon="el-icon-s-operation" @GetUserDisplay="getMoreInfo" />
          </div>
        </el-form>
        <div>
    ...
    <!-- 新增弹框 -->
    <modifyDialog v-model="modifyVisible" :add-sign="addSign" :form="form" :template-info="templateInfo" :data="dictObject" @getList="QueryStudents" />
    <!-- 详情弹框 -->
...
import { QueryStudents, DelStudent } from '@/api/UserInfo'
...
export default {
  ...
  filters: {
    tojson(val) {
      ...
      if (typeof JSON.parse(val) == 'object' && show != null) {
        show = show.join(',')
      }
      return show
    }
  ...
  methods: {
    ...
    getCondition(data) {
      // 选择框类型
      // { key: 'userlabels', name: '标签', list: '' },
      const selectArr = [
        { key: 'UserDetails.Politics', name: '政治面貌', list: 'ZZMM' },
        { key: 'StudentInfo.StuSatus', name: '学生状态', list: 'XSDQZT' },
        {
        ...
        { key: 'StudentInfo.StuCategory', name: '学生类别', list: 'XSLB' },
        { key: 'UserDetails.NativePlace', name: '籍贯', list: 'NATIVEPLACE' },
        { key: 'StudentInfo.StudyingYear', name: '学制', list: 'XZ' }
      ]
      // 时间类型
      const selectDate = [
        { key: 'UserDetails.Birthday', name: '生日' },
        { key: 'StudentInfo.StartSchoolTime', name: '入学时间' },
        { key: 'StudentInfo.GraduateTime', name: '毕业时间' }
      ]
      const allCondition = [...selectArr, ...selectDate]
      console.log(this.dictObject)
      if (data.length) {
        data.map((v, i) => {
          ...
          allCondition.map(item => {
            ...
            if (status) {
              ...
              } else if (selectDate.includes(item)) {
                v.ControlType = 6
              }
            }
          })
